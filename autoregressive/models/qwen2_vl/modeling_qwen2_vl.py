...
class Qwen2UnderGenConfig(Qwen2VLConfig):
    model_type = "Qwen2UnderGen"
    vision_vocab_size = 32768
...
# =============================================
class Qwen2UnderGenModel(Qwen2VLModel):

    config_class = Qwen2UnderGenConfig
    def __init__(self, config: Qwen2VLConfig):
    ...
    def forward(
        ...
        # video vqvae  
        input_vision_ids: torch.LongTensor = None,
        inputs_vision_embeds: Optional[torch.FloatTensor] = None, # add due to adding vision tokens
        attention_mask_matrix: Optional[torch.Tensor] = None, # need to update due to adding vision tokens
        ...
        # concatenate text and vqvae embeddings
        ...
        if attention_mask_matrix == None:
            causal_mask = self._update_causal_mask(
                attention_mask, inputs_embeds, cache_position, past_key_values, output_attentions
            ) # 
        else:
            assert attention_mask is None, "attention_mask should be None if attention_mask_matrix is not None"
            causal_mask = attention_mask_matrix
...
# =============================================
class Qwen2UnderGen(Qwen2VLForConditionalGeneration):
    
    config_class = Qwen2UnderGenConfig
    _tied_weights_keys = ["lm_head.weight", "lm_vision_head.weight"]

    def __init__(self, config):
        super(Qwen2VLForConditionalGeneration, self).__init__(config)
        # =================
        # 判断 config 中是否有 vqvae的vision_vocab_size，没有就设置一个默认值
        if not hasattr(config, "vision_vocab_size"):
            config.vision_vocab_size = 2048  # 你可以根据需要设置默认值
        ...
        # self.model = Qwen2VLModel(config)
        self.model = Qwen2UnderGenModel(config)
        self.vocab_size = config.vocab_size
        self.lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)
        self.padding_side = "left"  # set it to left by default, user can use setter to change padding_sides
        ...
        self.lm_vision_head = nn.Linear(config.hidden_size, config.vision_vocab_size, bias=False)
    ...
    def forward(
        ...
        #  video vqvae  
        input_vision_ids: torch.LongTensor = None,
        inputs_vision_embeds: torch.LongTensor = None,
        ...
        if inputs_embeds is None:
            inputs_embeds = self.model.embed_tokens(input_ids) # contains N <|video_pad|> tokens

            # image understanding 
            if pixel_values is not None:
                # vqvae video idx for generation
                assert input_vision_ids is None, 'Understanding and Generation conflicts!'
                pixel_values = pixel_values.type(self.visual.get_dtype())
                image_embeds = self.visual(pixel_values, grid_thw=image_grid_thw)
                n_image_tokens = (input_ids == self.config.image_token_id).sum().item()
                n_image_features = image_embeds.shape[0]
            ...
            # video(image) generaion, labels only contain visual tokens.
            enable_generation_func = False
            num_logits_to_keep = 0
            if input_vision_ids is not None or inputs_vision_embeds is not None:
                enable_generation_func = True
                # train
                if labels is not None:
            ...
            if attention_mask_matrix is not None:
                attention_mask_matrix = attention_mask_matrix.to(inputs_embeds.device)
            if attention_mask is not None:
                attention_mask = attention_mask.to(inputs_embeds.device)
        ...
        outputs = self.model(
            ...
            inputs_vision_embeds=inputs_vision_embeds,
