...
# FIXME 
class Qwen2VisionConfig(Qwen2Config):
    model_type = "qwen2Vision"
...
class Qwen2VisionModel(Qwen2Model):

    config_class = Qwen2VisionConfig
    ...
    def forward(
        ...
        attention_mask: Optional[torch.Tensor] = None, # need to update due to adding vision tokens
        position_ids: Optional[torch.LongTensor] = None, # need to update due to adding vision tokens
        past_key_values: Optional[List[torch.FloatTensor]] = None,
        inputs_embeds: Optional[torch.FloatTensor] = None,
        use_cache: Optional[bool] = None,
        output_attentions: Optional[bool] = None,
        ...
        #  FIXME create position embeddings to be shared across the decoder layers
        position_embeddings = self.rotary_emb(hidden_states, position_ids)
        ...
        for decoder_layer in self.layers:
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = decoder_layer(
                    hidden_states,
                    attention_mask=causal_mask, # need to update this
                    position_ids=position_ids, # need to update this
                    past_key_value=past_key_values,
                    output_attentions=output_attentions,
                    use_cache=use_cache,
                    cache_position=cache_position,
                    position_embeddings=position_embeddings, # need to update this
                )
...
class Qwen2VisionForCausalLM(Qwen2ForCausalLM):
